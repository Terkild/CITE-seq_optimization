...
outdir <- "C:/Users/Terkild2/OneDrive - Københavns Universitet/Koralovlab/ECCITE-seq/20200106 Titration 1"
```
...
p.ADTcount.AbtitrationByConc <- ggplot(ADT.matrix.agg.total[order(-ADT.matrix.agg$conc, -ADT.matrix.agg$sum),], aes(x=dilution, y=sum/10^6, fill=conc)) + 
...
data.ADT.DSB.above5 <- as.data.frame(data.ADT.DSB) %>% 
                          mutate(marker=rownames(.)) %>% 
                          pivot_longer(-marker) %>% 
                          filter(data.meta[name,"dilution"]=="DF1" & value >= markerStats[paste(marker,data.meta[name,"tissue"],sep="_"),"DSB.cutoff"]) %>%
                          group_by(marker, fineCluster=data.meta[name,"fineCluster"]) %>% 
                          summarize(count=length(value), mean=mean(value), sd=sd(value), median=median(value), nth=nth(value)) %>% 
                          arrange(fineCluster, marker)

negativeMarkers <- c(setdiff(rownames(data.ADT.DSB),unique(data.ADT.DSB.above5$marker)))

#negativeMarkers.df <- data.frame("marker"=negativeMarkers)
#negativeMarkers.df[,c("fineCluster","count","mean","sd","median","nth")] <- 0
#data.ADT.DSB.above5 <- rbind(as.data.frame(data.ADT.DSB.above5),negativeMarkers.df)

data.ADT.DSB.above5.matrix <- data.ADT.DSB.above5 %>% 
...
data.ADT.DSB.above5.count.matrix <- data.ADT.DSB.above5 %>% 
                                select(marker, fineCluster, count) %>% 
                                mutate(count=scales::rescale(count,to=c(0, 1))) %>% 
...
data.ADT.DSB.above5.freq.matrix <- data.ADT.DSB.above5 %>% 
                                select(marker, fineCluster, count) %>% 
...
col_fun = circlize::colorRamp2(c(0, 5, 20, 25), c("blue", "cyan", "yellow", "red"))
...
ann.conc <- log2(abpanel[colnames(data.ADT.DSB.above5.matrix),"conc_µg_per_mL"])
...
ann.fineCluster <- data.meta %>% 
  ...
  filter(count > 1 & fineCluster %in% rownames(data.ADT.DSB.above5.matrix)) %>%
  tibble::column_to_rownames("fineCluster")
...
column_ha = HeatmapAnnotation("UMI count (PBMC)"=ann.UMIuse.PBMC[colnames(data.ADT.DSB.above5.matrix),"sum"]/10^3, 
                              "UMI count (Lung)"=ann.UMIuse.Lung[colnames(data.ADT.DSB.above5.matrix),"sum"]/10^3,
                              "DF1 conc."=ann.conc, 
...
## Make Circle function for ComplexHeatmap
drawCircle <- function(j, i, x, y, width, height, fill){
  selected <- which(Cluster.max[,"fineCluster"] == rownames(data.ADT.DSB.above5.matrix)[i] & Cluster.max[,"marker"]==colnames(data.ADT.DSB.above5.matrix)[j])
  if(length(selected) > 0){
    grid.rect(x=x, y=y, width=width, height=height, gp=gpar(col=color.tissue[Cluster.max[selected,"tissue"]], 
                                                            fill=NA 
#, lty=ifelse(Cluster.max[selected,"tissue"]=="PBMC","solid","dashed")
                                                           ))
  }
  radius <- data.ADT.DSB.above5.freq.matrix[i, j]*1.8
  ## If positive population is less than 2 percent, do not show it
  radius <- ifelse(radius > 0.02*1.8,radius+0.2, 0)
  grid.circle(x = x, y = y, r = radius*min(unit.c(width, height)),gp = gpar(fill = col_fun(data.ADT.DSB.above5.matrix[i, j]), col=alpha("black",0.5), size=0.1))
}

# Add UMI usage in PBMC/Lung for columns or remove Ab conc label (or move to left?)
# Move color legend to upper left corner (or redraw with ggplot/cowplot)
# Automatic "gates" on titration plots by finding "rank" having DSB > 5?

ht_list <- Heatmap(data.ADT.DSB.above5.matrix, 
                   cell_fun=drawCircle, 
...
p2.legend <- cowplot::get_legend(p2 + theme(legend.position="bottom", legend.direction="horizontal", legend.background=element_blank(), legend.box.background=element_blank(), legend.key=element_blank()))
cowplot::plot_grid(p2.legend)

p.heatmap <- grid::grid.grabExpr(draw_heatmap_now(), wrap=TRUE, wrap.grobs=FALSE, height=3.1, width=6.99)
p.heatmap.legend <- cowplot::get_legend(ggplot(data.frame(values=c(0, 7.5, 15, 22.5, 30)),aes(x=values,y=values,col=values)) + scale_color_gradientn(colours=col_fun(c(0, 7.5, 15, 22.5, 30)), values=c(0, 7.5, 15, 22.5, 30)/30, breaks=c(0,30),labels=c("Low","High")) + geom_point() + theme(legend.title=element_blank(), legend.background=element_blank(), legend.box.background=element_blank(), legend.key=element_blank(), legend.key.height=unit(2,"mm"), legend.key.width=unit(3,"mm")))
...
FG <- cowplot::plot_grid(E1, E2, E1, F1, nrow=1, align="h", axis="tb", labels=c("F", "", "G", ""), label_size=panel.label_size, vjust=panel.label_vjust, hjust=panel.label_hjust, rel_widths=c(2,10,2,10))

png(file=file.path(outdir,"Figure 1.png"), width=figure.width.full, height=9.5, units = figure.unit, res=figure.resolution, antialias=figure.antialias)
cowplot::ggdraw(cowplot::plot_grid(AD, p.heatmap, FG, ncol=1, rel_heights=c(2.2,3.4,4.8), align="v", axis="l", labels=c("", "E", ""), label_size=panel.label_size, vjust=panel.label_vjust, hjust=panel.label_hjust)) + cowplot::draw_plot(p2.legend,0.08,0.017,0.2,0.00001) + cowplot::draw_plot(p.heatmap.legend,1,0.47,0.12,0.001,hjust=1, vjust=0)
dev.off()
```
...
p.CD30 <- foot_plot_density_custom(marker=paste0("adt_",curMarker.name), conc=curMarker.DF1conc, legend=FALSE, yaxis.text=TRUE)

curMarker.name <- "CD183"
curMarker.DF1conc <- abpanel[curMarker.name, "conc_µg_per_mL"]

p.CD183 <- foot_plot_density_custom(marker=paste0("adt_",curMarker.name), conc=curMarker.DF1conc, legend=FALSE)

curMarker.name <- "TCRgd"
curMarker.DF1conc <- abpanel[curMarker.name, "conc_µg_per_mL"]

p.TCRgd <- foot_plot_density_custom(marker=paste0("adt_",curMarker.name), conc=curMarker.DF1conc, legend=FALSE)

A <- cowplot::plot_grid(p.CD30, p.CD183, p.TCRgd, nrow=1)
...
p.HLADR <- foot_plot_density_custom(marker=paste0("adt_",curMarker.name), conc=curMarker.DF1conc, gates=gate, legend=FALSE, yaxis.text=TRUE)

B <- cowplot::plot_grid(p.HLADR, p.CD4, p.CD19, nrow=1)
...
p.CD3 <- foot_plot_density_custom(marker=paste0("adt_",curMarker.name), conc=curMarker.DF1conc, gates=gate, legend=FALSE, yaxis.text=TRUE)
...
C <- cowplot::plot_grid(p.CD3, p.CD14, p.CD39, nrow=1)
...
p.HLA <- foot_plot_density_custom(marker=paste0("adt_",curMarker.name), conc=curMarker.DF1conc, legend=FALSE, yaxis.text=TRUE)
...
D <- cowplot::plot_grid(p.HLA, p.CD44, p.CD45, nrow=1)
...
p.CD56 <- foot_plot_density_custom(marker=paste0("adt_",curMarker.name), conc=curMarker.DF1conc, legend=FALSE, yaxis.text=TRUE)

curMarker.name <- "CD117"
curMarker.DF1conc <- abpanel[curMarker.name, "conc_µg_per_mL"]

p.CD117 <- foot_plot_density_custom(marker=paste0("adt_",curMarker.name), conc=curMarker.DF1conc, legend=FALSE)

curMarker.name <- "CD127"
curMarker.DF1conc <- abpanel[curMarker.name, "conc_µg_per_mL"]

p.CD127 <- foot_plot_density_custom(marker=paste0("adt_",curMarker.name), conc=curMarker.DF1conc, legend=FALSE)

E <- cowplot::plot_grid(p.CD56, p.CD117, p.CD127, nrow=1)


# a bit of a hack to get celltype legend
p.withLegend <- ggplot(data.frame(supercluster=object$supercluster),aes(color=supercluster,x=1,y=1)) + geom_point(shape=15, size=1.5) + scale_color_manual(values=color.supercluster)
...
png(file=file.path(outdir,"Figure 2.png"), units=figure.unit, res=figure.resolution, width=figure.width.full, height=10, antialias=figure.antialias)
cowplot::plot_grid(NULL, p.legend, A, NULL, B, NULL, C, NULL, D, NULL, E, labels=c("","","A","","B","","C","","D","","E"), vjust=-0.5, hjust=panel.label_hjust, label_size=panel.label_size, ncol=1, rel_heights= c(1.3,1.5,20,1.5,20,1.5,20,1.5,20,1.5,20))
dev.off()
