---
title: "CITE-seq optimization - Antibody concentration titration"
...
Subset to only focus on conditions with 50µl staining volume (thus comparing DF1 to DF4) in both PBMC and lung samples.
...
Get data and calculate expression level and frequencies within positive cells from each cluster
...
Heatmap showing the fraction of positive cells (circle size) and their expression level (color) of each antibody within each cluster. Including additional annotation such as UMI counts within each tissue and antibody concentration.
...
```{r Figure1, fig.height=9.5, fig.width=7}
...
Reduction of antibody concentration had different inpacts on different markers. We divided the markers into five categories and show examples of each category below:

```{r titrationExamples, fig.height=10, fig.width=7}
