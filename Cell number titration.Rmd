...
p2 <- ggplot(ADT.matrix.agg, aes(x=marker.byConc,y=log2(sum))) + 
  ...
  ylab("log2(UMI sum)") + 
  guides(fill=guide_legend(override.aes=list(size=1.5), reverse=FALSE)) + 
...
cowplot::ggdraw(plot_grid(ADE, BC, nrow=1, rel_widths=c(1,4), align="v", axis="l")) + cowplot::draw_plot(p2.legend,0.27,0.027,0.2,0.00001)
dev.off()
```
